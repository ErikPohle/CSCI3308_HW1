...
function begin_play(){
	...
	if (p2.value.length == 0 || p1.value.length == 0) {
...
function play() {
	...
	var possibleMoves = ["", "A1", "A2", "A3", "B1", "B2", "B3", "C1", "C2", "C3"];
	var subset = ["X", "O"];
	document.getElementById("move_text_id").value = "";
	if (possibleMoves.includes(move) && started) {
			if(move == "") {
				alert("The game has not started.");
				return;
			}
			if(document.getElementById(move).innerHTML == "O" || document.getElementById(move).innerHTML == "X") {
				alert("This cell has already been played on. Please pick another cell.");
				return;
			}
			document.getElementById(move).innerHTML = whose_move() ? "X" : "O";
			toggle_move();

			// validate game board for Winner
			if(document.getElementById("A1").innerHTML == "X" && document.getElementById("A2").innerHTML == "X" && document.getElementById("A3").innerHTML == "X") {
				alert("Winner is : X");
				reset_play();
				return;
			}
			else if(document.getElementById("A1").innerHTML == "O" && document.getElementById("A2").innerHTML == "O" && document.getElementById("A3").innerHTML == "O") {
				alert("Winner is : O");
				reset_play();
				return;
			}
